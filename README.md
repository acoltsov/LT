...
- **Interactive gridlines**: the blueprint grid in the hero, reputation, and waitlist sections is a canvas (`src/components/ui/WarpGrid.tsx`) that behaves like a spacetime fabric — the cursor acts as a dense mass (with a soft lime glow riding on it) and nearby lines are pulled inward, gravity-well style, springing back when it leaves. On touch, press and hold: the longer you hold, the deeper, wider and brighter the well gets (scrolling releases it). It renders a static grid under `prefers-reduced-motion` and only animates while on-screen.
