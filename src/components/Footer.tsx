...
function StoreButton({
  ...
  return (
    <div
      className="flex cursor-not-allowed items-center gap-2.5 rounded-xl border border-slate-700 px-4 py-2.5 opacity-80"
      title="Coming soon"
    >
      <Icon className="size-5 text-slate-300" aria-hidden="true" />
      <div className="text-left leading-tight">
        <p className="font-mono text-[9px] tracking-widest text-lime-300 uppercase">
          Coming soon
        </p>
        <p className="text-sm font-semibold text-slate-200">{store}</p>
      </div>
    </div>
  );
}
...
export function Footer() {
  return (
    <footer className="bg-slate-950 dark:bg-[#070c18]">
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr_1.4fr]">
          <div>
            <a href="#top" className="flex items-center gap-2" aria-label="LinkTrades — back to top">
              ...
              <span className="font-display text-lg font-extrabold tracking-tight text-white">
                Link<span className="text-lime-400">Trades</span>
              </span>
            </a>
          ...
          <nav aria-label="Product">
            <h3 className="font-mono text-xs font-semibold tracking-[0.2em] text-slate-400 uppercase">
              Product
            </h3>
